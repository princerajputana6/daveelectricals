...
export async function POST(req: Request) {
  try {
    ...
    const to = process.env.CONTACT_TO || "info@davecleaning.co.uk";
...
— sent from davecleaning.co.uk
Submission ID: ${result.insertedId.toString()}`;

    const html = `
      <div style="font-family:system-ui,sans-serif;max-width:600px">
        <div style="background:#050505;color:#1877d1;padding:16px 22px;font-weight:700;font-size:18px">
          Dave Cleaning — Website Enquiry
        </div>
        <div style="border:1px solid #eee;border-top:0;padding:22px;color:#111">
          ...
          <div style="margin-top:18px;padding:14px;background:#fafafa;border-left:3px solid #1877d1;white-space:pre-wrap">${submission.message}</div>
