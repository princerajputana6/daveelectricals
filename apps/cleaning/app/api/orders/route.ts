...
export async function POST(req: Request) {
  try {
        ...
        line_items: [
          {
            ...
            price_data: {
              ...
              product_data: {
                name: isBooking
                  ? "Dave Cleaning — same-day emergency booking"
                  : paymentMode === "full"
                    ? "Dave Cleaning — full payment"
                    : "Dave Cleaning — 50% deposit",
                description: booking
    ...
    try {
      const notifyTo =
        process.env.ORDER_NOTIFICATION_EMAIL || "info@davecleaning.co.uk";
