...
export async function PATCH(
  ...
  if (issuedCert && order.customer?.email) {
    ...
    const subject = `Your ${issuedCert.type} has been issued — Dave Cleaning`;
    const text = `Hi ${order.customer.name.split(" ")[0]},

Your ${issuedCert.type} (# ${issuedCert.number}) has been issued by Dave Cleaning Services.
...
Thank you for choosing Dave Cleaning Services.
    ...
    const html = `
      <div style="font-family:system-ui,sans-serif;max-width:600px;margin:auto">
        <div style="background:#050505;color:#e2e61f;padding:18px 24px;font-weight:800;font-size:18px;letter-spacing:.5px">
          DAVE CLEANING · CERTIFICATE ISSUED
        </div>
        <div style="border:1px solid #eee;border-top:0;padding:24px;color:#111;line-height:1.55">
          ...
          <p style="color:#666;font-size:12px;margin-top:24px">Dave Cleaning Services · ${company.phonePrimary} · ${company.email}</p>
        </div>
      </div>`;
