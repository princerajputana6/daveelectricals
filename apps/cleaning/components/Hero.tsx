...
const headline = ["Cleaning you", "can rely on"];
...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_30%,rgba(24,119,209,0.14),transparent_55%)]" />
      ...
      >
        ...
        >
          ...
          <span className="text-[11px] font-semibold uppercase tracking-[0.25em] text-bolt sm:text-xs">
            Domestic · Commercial · End of Tenancy Cleaning
          </span>
        </motion.div>
          ...
          {headline.map((line, li) => (
            <span key={line} className="block overflow-hidden">
              <motion.span
                ...
                className={`block whitespace-nowrap ${li === 1 ? "text-bolt" : "text-slate-900"}`}
              >
                {line}
              </motion.span>
            </span>
        ...
        >
          Reliable domestic and commercial cleaning across London, delivered with attention to detail, flexible appointments and a professional service from your first enquiry to completion.
        </motion.p>
        ...
        >
          <Link
            href="/contact"
            className="group flex items-center gap-2 rounded-full bg-bolt px-7 py-3.5 font-bold text-white transition-transform hover:scale-[1.04]"
          >
            Get a free quote
          ...
          <a
            ...
            className="group flex items-center gap-2 rounded-full border border-slate-200 bg-slate-100 px-7 py-3.5 font-semibold text-slate-900 backdrop-blur-sm transition-colors hover:border-bolt/40 hover:text-bolt"
          >
        ...
        >
          <div className="flex items-center gap-1.5">
            ...
            <span className="ml-1 text-sm text-slate-500">Trusted local cleaners</span>
          </div>
          {badges.map((b) => (
            <span
              key={b}
      ...
      >
        <div className="flex h-11 w-7 items-start justify-center rounded-full border border-slate-200 p-1.5">
          <motion.span
            animate={{ y: [0, 12, 0] }}
