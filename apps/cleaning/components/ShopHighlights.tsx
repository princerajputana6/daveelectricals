...
function ProductCard({ product, index }: { product: Product; index: number }) {
  ...
  return (
    <motion.article
      ...
      className="group relative flex flex-col overflow-hidden rounded-3xl border border-bolt/20 bg-gradient-to-br from-white via-slate-50 to-white p-7"
    >
      ...
      {product.badge && (
        <span className="relative w-fit rounded-full bg-bolt px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-white">
          {product.badge}
        </span>
      )}
      ...
      <div className="relative mt-4 flex items-start justify-between gap-4">
        <div>
          ...
          <h3 className="mt-2 font-display text-xl font-bold leading-tight text-slate-900 sm:text-2xl">
            {product.name}
          </h3>
        </div>
      ...
      {product.id === "same-day-call-out" ? (
        <div className="relative mt-6 flex flex-wrap items-end justify-between gap-4 border-t border-slate-200 pt-5">
          <div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Link
              href="/book"
              className="group flex items-center gap-2 rounded-full bg-bolt px-6 py-3 font-bold text-white transition-transform hover:scale-[1.04] no-underline"
            >
              Check availability
            ...
            <a
              ...
              className="flex items-center gap-2 rounded-full border border-slate-200 bg-slate-100 px-6 py-3 font-bold text-slate-900 no-underline transition-colors hover:border-bolt/40 hover:text-bolt"
            >
        ...
        <div className="relative mt-6 flex flex-wrap items-end justify-between gap-4 border-t border-slate-200 pt-5">
          ...
          <a
            href={`tel:${company.phonePrimary}`}
            className="flex items-center gap-2 rounded-full bg-bolt px-6 py-3 font-bold text-white no-underline transition-transform hover:scale-[1.04]"
          >
            <PhoneIcon className="h-4 w-4" />
            Call for a quote
          </a>
          ...
          {requiresVariant && (
            <div className="relative mt-5">
              ...
              <select
                ...
                className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 focus:border-bolt/60 focus:outline-none focus:ring-2 focus:ring-bolt/20"
              >
