...
export default function AdminOrderEditor({ order }: { order: OrderPublic }) {
  ...
  const [cert, setCert] = useState({
    ...
    type: order.certificate?.type || "EICR — Landlord Cleaning Safety Certificate",
  ...
  const input =
    "w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-500 focus:border-bolt/60 focus:outline-none focus:ring-2 focus:ring-bolt/20";
  const label =
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1.5fr_1fr]">
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
            ...
            {order.customer.preferredDate && (
              <div>
                <p className="text-xs text-slate-500">Preferred visit date</p>
            ...
            {order.customer.keyCollection && (
              <div className="sm:col-span-2">
                <p className="text-xs text-slate-500">Key collection</p>
                <p className="whitespace-pre-line text-slate-600">
                  {order.customer.keyCollection}
                </p>
              </div>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6">
          <h2 className="font-display text-lg font-bold text-slate-900">Items</h2>
          ...
          <dl className="mt-4 space-y-1 text-sm">
            <div className="flex justify-between">
              <dt className="text-slate-500">Subtotal (ex VAT)</dt>
              <dd className="text-slate-900">{formatGBP(order.subtotal)}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-slate-500">VAT ({order.vatRate ?? 0}%)</dt>
              <dd className="text-slate-900">{formatGBP(order.vatAmount ?? 0)}</dd>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              <dt className="text-slate-500">Deposit (50%)</dt>
              <dd
                className={
            ...
            <div className="flex justify-between">
              <dt className="text-slate-500">Balance (50%)</dt>
              <dd
                className={
                  balancePaid
                    ? "text-emerald-300"
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className="rounded-full border border-slate-200 bg-slate-100 px-5 py-2 text-sm font-semibold text-slate-900 hover:border-bolt/40 hover:text-bolt disabled:opacity-50"
            >
            ...
            <button
              ...
              onClick={issueCert}
              disabled={busy === "cert"}
              className="group flex items-center gap-2 rounded-full bg-bolt px-6 py-2.5 text-sm font-bold text-white hover:scale-[1.04] disabled:opacity-50"
            >
      ...
      <aside className="space-y-5">
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <button
            ...
            className="mt-3 w-full rounded-xl bg-bolt px-4 py-2.5 text-sm font-bold text-white hover:scale-[1.02] disabled:opacity-50"
          >
        ...
        {!balancePaid && order.paymentMode !== "full" && (
          <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-6">
            ...
            <button
              ...
              className="mt-4 w-full rounded-xl bg-amber-400 px-4 py-2.5 text-sm font-bold text-white hover:scale-[1.02] disabled:opacity-50"
            >
              {busy === "balance"
                ? "Saving…"
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <dl className="mt-3 space-y-1.5 text-xs">
            <div className="flex justify-between gap-3">
              ...
              <dd className="text-slate-900">{order.invoiceNumber || "—"}</dd>
            </div>
            <div className="flex justify-between gap-3">
          ...
          <button
            ...
            className="mt-4 w-full rounded-full border border-slate-200 bg-slate-100 px-4 py-2 text-xs font-semibold text-slate-900 transition-colors hover:border-bolt/40 hover:text-bolt disabled:opacity-50"
          >
