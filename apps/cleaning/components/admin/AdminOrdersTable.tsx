...
const STATUS_TONE: Record<OrderPublic["status"], string> = {
  ...
  cancelled: "bg-zinc-500/15 text-slate-500",
};

const DEFAULT_CERT_TYPES = [
  "EICR — Landlord Cleaning Safety Certificate",
          ...
          {orders.length === 0 ? (
            ...
            orders.map((o) => {
              ...
              return (
                <Fragment key={o.id}>
                  <tr
                    className="border-t border-slate-100 transition-colors hover:bg-white/[0.03]"
                  >
                    <td className="px-5 py-3">
                      <Link
                        href={`/admin/orders/${o.id}`}
                        className="font-display font-bold text-slate-900 hover:text-bolt"
                      >
                    ...
                    <td className="px-5 py-3 text-xs">
                      <p
                        className={
                          depositPaid ? "text-emerald-300" : "text-slate-500"
                        }
                      >
                    ...
                    <td className="px-5 py-3">
                      <div className="flex flex-wrap justify-end gap-1.5">
                        ...
                        <Link
                          href={`/admin/orders/${o.id}`}
                          className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-900 hover:bg-slate-100"
                        >
                          Open
                        </Link>
                      </div>
                  ...
                  <AnimatePresence>
                    {open && (
                      <tr>
                        <td colSpan={7} className="bg-white/60 p-0">
                          <motion.div
...
function CertForm({
  ...
  const input =
    "w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-500 focus:border-bolt/60 focus:outline-none focus:ring-2 focus:ring-bolt/20";
  const label =
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
      <div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className="rounded-full border border-slate-200 bg-slate-100 px-4 py-2 text-xs font-semibold text-slate-900 hover:border-bolt/40 hover:text-bolt disabled:opacity-50"
          >
          ...
          <button
            ...
            disabled={busy === "issue"}
            onClick={issue}
            className="group flex items-center gap-2 rounded-full bg-bolt px-5 py-2 text-xs font-bold text-white hover:scale-[1.04] disabled:opacity-50"
          >
            {busy === "issue"
              ? "Saving…"
              : order.certificate
      ...
      {/* Summary aside */}
      <aside className="rounded-xl border border-slate-200 bg-white p-4 text-xs">
...
function defaultCertTypeFor(order: OrderPublic) {
  ...
  if (ids.has("eicr")) return "EICR — Landlord Cleaning Safety Certificate";
