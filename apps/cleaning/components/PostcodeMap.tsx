...
export default function PostcodeMap() {
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-bolt/30 shadow-2xl shadow-bolt/10">
      {/* Top ribbon */}
      <div className="flex items-center justify-between gap-3 bg-bolt px-6 py-3 text-white sm:px-8">
        ...
        <div className="flex flex-wrap gap-x-5 gap-y-2">
          {LEGEND.map((l) => (
            ...
            >
              ...
              />
              <span className="font-semibold text-slate-900">{l.area}</span>
              <span className="text-slate-500">{l.label}</span>
            </span>
          ))}
          <span className="text-xs text-slate-500">…and every district in between.</span>
        </div>
      </div>
    </div>
  );
