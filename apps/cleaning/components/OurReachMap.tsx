...
const locations = [
  {
    ...
    lat: 51.4677,
    lng: -0.361,
    description: "Our main base of operations — full cleaning services for domestic & commercial clients.",
  },
  {
    name: "Twickenham",
    lat: 51.4489,
  ...
  },
  {
    ...
    lat: 51.4473,
    lng: -0.4082,
    description: "Commercial and industrial cleaning work throughout the Feltham area.",
  },
  {
    name: "Cranford",
    lat: 51.4856,
    ...
    <section className="bg-white">
      <div className="mx-auto max-w-7xl px-5 py-16 sm:px-8 sm:py-20">
        <div className="grid gap-10 lg:grid-cols-[1fr_360px]">
          {/* Map */}
          <div className="overflow-hidden rounded-2xl border border-slate-200 shadow-xl" style={{ minHeight: 480 }}>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="mt-2 flex flex-col gap-3">
              {locations.map((loc) => (
                <div
                  key={loc.name}
                  className="flex items-start gap-4 rounded-xl border border-slate-200 bg-white p-4 transition-colors hover:border-bolt/40"
                >
                  <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-bolt font-display text-xs font-bold text-white">
                    {loc.name[0]}
                  </span>
                  <div>
                    <p className="font-display text-sm font-bold text-slate-900">{loc.name}</p>
                    <p className="mt-0.5 text-xs leading-relaxed text-slate-500">{loc.description}</p>
                  </div>
                </div>
              ))}
            </div>
