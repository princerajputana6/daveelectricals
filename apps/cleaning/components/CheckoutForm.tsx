  ...
  if (count === 0) {
    return (
      <div className="rounded-3xl border border-slate-200 bg-white p-10 text-center">
        ...
        <Link
          href="/services"
          className="mt-6 inline-flex items-center gap-2 rounded-full bg-bolt px-6 py-3 font-bold text-white"
        >
          Browse services
      ...
      <div className="grid gap-8 lg:grid-cols-[1.5fr_1fr]">
        ...
        >
          ...
          <fieldset className="mt-7">
            <legend className="mb-3 block text-xs font-semibold uppercase tracking-wider text-slate-500">
              Payment option
            </legend>
            <div className="grid gap-3 sm:grid-cols-2">
              <label
                className={`relative cursor-pointer rounded-2xl border p-4 transition-all ${
                  paymentMode === "split"
                    ...
                    : "border-slate-200 bg-white hover:border-bolt/40"
                }`}
              >
                <input
                  type="radio"
                ...
                <div className="flex items-center justify-between">
                  <span
                    className={`font-display text-base font-bold ${
                      paymentMode === "split" ? "text-bolt" : "text-slate-900"
                    }`}
                  >
                    50% deposit
                  </span>
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <span
                    className={`font-display text-base font-bold ${
                      paymentMode === "full" ? "text-bolt" : "text-slate-900"
                    }`}
                  >
                    Pay in full
                  </span>
          ...
          <dl className="mt-6 space-y-1.5 rounded-2xl border border-slate-200 bg-white p-4 text-sm">
          ...
          <button
            type="submit"
            disabled={pending}
            className="group mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-bolt px-6 py-3.5 font-bold text-white transition-transform hover:scale-[1.02] disabled:cursor-not-allowed disabled:opacity-60"
          >
            {pending
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-7">
            ...
            <ul className="mt-4 space-y-3 text-sm">
              {lines.map((l) => (
                <li
                  key={l.key}
                  className="flex items-start justify-between gap-3 border-b border-slate-100 pb-3 last:border-b-0"
                >
                  <div>
